...
class StartMMMFarming:
    ...
    async def handle_app_behavior(self, is_start):
        if is_start:
            ...
            os.system(f"adb -s {self.device_id} shell input tap 955 635")
    ...
    async def tap_farming(self):
        for _ in range(3):
            os.system(f"adb -s {self.device_id} shell input tap 515 1890")
            await asyncio.sleep(3)
        await asyncio.sleep(5)
    ...
    async def start_MMM(self):
        ...
        while True:
            print("starting Mr. MEME .....")
            try:
                dbContext = mysqlDBqueries()
                dbContext.insert_process_data('mmm', True, 999)
            except Exception as e:
                print(f"DB exception: {e}")
